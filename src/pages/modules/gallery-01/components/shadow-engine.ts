...
export class ShadowEngine {
  ...
  private calculateShadowFromLight(
    ...
    const shadowDirection = this.projectShadowTo2D(
      lightDirection,
      photo.position.z,
    );

    // Shadow distance based on height and light angle
    const shadowDistance = this.calculateShadowDistance(
      photo.position.z,
      lightDirection,
    );
    ...
    const shadowRotation =
      photo.rotation * 0.7 +
      this.calculateLightInfluenceOnRotation(lightDirection);
  ...
  private calculateShadowDistance(
    objectHeight: number,
    lightDirection: Vector3D,
  ): number {
    ...
    const angleFactor =
      Math.abs(lightDirection.z) < 0.1 ? 5 : 1 / Math.abs(lightDirection.z);
  ...
  private calculateLightInfluenceOnRotation(lightDirection: Vector3D): number {
    ...
    const angle =
      Math.atan2(lightDirection.y, lightDirection.x) * (180 / Math.PI);
    return angle * 0.1; // Subtle influence
  }

  private calculateShadowColor(light: LightSource): {
    r: number;
    g: number;
    b: number;
  } {
  ...
  private combineShadows(
    shadow1: ShadowProperties,
    shadow2: ShadowProperties,
  ): ShadowProperties {
